...
from .catboost_dynamic import CatboostDynamicTrainer
...
class LSTMCascadeExtendModel(nn.Module):
    ...
    def forward(self, x:torch.Tensor, mix=False):
        ...
        if mix:
            ...
            gbdt_out = torch.reshape(torch.as_tensor(gbdt_out, dtype=torch.float32), (b,t,4)).to(x.device)
        # mask: (batch, time)
        x = self.norm(x)
        # x: (batch, feature, time)
...
class LSTMCascadeExtendTrainer():
    ...
    def train(self, addi_params:dict=None):
        ...
        self.model = self.model.to(self.device)
        self.criterion = self.criterion.to(self.device)
        self.catboost_trainer.train(addi_params=addi_params)
        self.model.set_catboost_model(self.catboost_trainer.model)
        # 更新dropout
        if addi_params is not None:
            if 'dropout' in addi_params.keys():
