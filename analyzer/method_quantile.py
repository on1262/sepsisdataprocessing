...
class LSTMQuantileAnalyzer:
    '''动态模型, 回归预测窗口内最小值'''
    def __init__(self, params:dict, container:DataContainer) -> None:
        self.params = params
        self.paths = params['paths']
        self.container = container
        self.model_name = 'LSTM_quantile'
    ...
    def run(self):
        ...
        metric = tools.QuantileRegressionMetric(target_name=self.container.target_name, out_dir=self.out_dir, taus=self.params['quantile_taus'])
        ...
        for idx, (data_index, test_index) in enumerate(kf.split(X=self.dataset)): 
            ...
            trainer = mlib.LSTMQuantileTrainer(self.params, self.dataset)
            if idx == 0:
                trainer.summary()
            trainer.train()
        ...
        self.loss_logger.plot(std_bar=False, log_loss=False, title='Loss for LSTM quantile Model', 
